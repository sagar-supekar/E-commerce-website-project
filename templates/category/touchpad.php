...
?>
 ...
 <div class='d-flex justify-content-start ms-5 my-2' style='margin-top=10px;'>
     <a href='/E-commerce website/templates/category/mobile_cart_templates.php' class='text-decoration-none'>
...
echo"<div class='d-flex justify-content-center my-2' ><h3>TOUCH PADS</h3></div>";
...
$results_per_page = 6;
$query = "SELECT * FROM e_product_details where subcategory='touchpad'";
...
$query = "SELECT * FROM e_product_details WHERE subcategory='touchpad' LIMIT $page_first_result, $results_per_page";
$result = mysqli_query($link, $query);
...
// When page = 1
if ($page > 1) {
    echo '<a href="touchpad.php?page=' . ($page - 1) . '">Previous</a>';
} else {
...
for ($i = 1; $i <= $number_of_page; $i++) {
    if ($i == $page) {
        echo '<a href="touchpad.php?page=' . $i . '" class="active">' . $i . '</a>';
    } else {
        echo '<a href="touchpad.php?page=' . $i . '">' . $i . '</a>';
    }
}

// If current page is less than page size, add one more page
if ($page < $number_of_page) {
    echo '<a href="touchpad.php?page=' . ($page + 1) . '">Next</a>';
} else {
