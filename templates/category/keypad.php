...
<?php
echo"<div class='d-flex justify-content-center my-2' ><h3>KEYPADS</h3></div>";
...
$results_per_page = 6;
$query = "SELECT * FROM e_product_details where subcategory='keypad'";
...
$query = "SELECT * FROM e_product_details WHERE subcategory='keypad' LIMIT $page_first_result, $results_per_page";
$result = mysqli_query($link, $query);
...
// When page = 1
if ($page > 1) {
    echo '<a href="keypad.php?page=' . ($page - 1) . '">Previous</a>';
} else {
...
for ($i = 1; $i <= $number_of_page; $i++) {
    if ($i == $page) {
        echo '<a href="keypad.php?page=' . $i . '" class="active">' . $i . '</a>';
    } else {
        echo '<a href="keypad.php?page=' . $i . '">' . $i . '</a>';
    }
}

// If current page is less than page size, add one more page
if ($page < $number_of_page) {
    echo '<a href="keypad.php?page=' . ($page + 1) . '">Next</a>';
} else {
